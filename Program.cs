...
namespace ConsoleGraphicEngine
{
    class Program
    {
        ...
        private static IScene InitializeScene()
        {
            ...
            camera.transform.Rotate(new Vector3(1, 0, 0), (float)Math.PI / 2);

            IObject3D globalLight = new Object3D("GlobalLight");
            globalLight.AddComponent(new GlobalLight(new Vector3(1, 1, -0.5f), 1));

            IObject3D sphere1 = new VisibleObject(new SphereObjectRenderer(1), "Sphere");
            sphere1.transform.position = new Vector3(0, 0, 0);

            IObject3D sphere2 = new VisibleObject(new SphereObjectRenderer(0.5f), "Sphere");
            sphere2.transform.position = new Vector3(1, 1, 1);
